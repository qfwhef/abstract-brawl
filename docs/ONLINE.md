...
`abstract-brawl` 的回合战与格斗模式均支持两名玩家通过 WebSocket 进行在线对战，共用同源 `/ws` 房间服务。格斗大厅支持公开/私密房间、房号加入、选角锁定、再战与重选角色。

两种模式的同步方式不同：回合战由服务端运行规则引擎并校验行动；格斗由客户端模拟，服务端转发输入与房主状态快照。以下架构、行动协议及完整战斗重放说明以回合战为主。
...
- 首次部署使用完整源码和 Node.js 22 或更新版本，执行 `npm ci --omit=dev` 安装 `ws`，再运行 `node server/static-server.mjs`。静态网页构建包不包含该服务。
- 网页与 `/ws` 需通过同一个站点访问；HTTPS 反向代理需支持 WebSocket 升级连接。
